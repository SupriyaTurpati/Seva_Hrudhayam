...
const protect = async (req, res, next) => {
  ...
  if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
    try {
      ...
      const decoded = jwt.verify(token, process.env.JWT_SECRET || 'seva_hrudhayam_secret_key_12345');
      
      if (decoded.role === 'admin') {
        req.user = await Admin.findById(decoded.id).select('-password');
      } else if (decoded.role === 'donor') {
        req.user = await Donor.findById(decoded.id).select('-password');
      } else if (decoded.role === 'orphanage_head') {
        req.user = await OrphanageHead.findById(decoded.id).select('-password');
      }
